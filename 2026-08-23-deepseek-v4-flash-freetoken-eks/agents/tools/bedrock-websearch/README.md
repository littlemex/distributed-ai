...
search tool, so a self-hosted model (DeepSeek-V4-Flash on FreeToken) stays the agent's "brain" while
...
agent's own tool-calling. This wrapper makes it a callable tool instead, so any agent
can use it without switching its model off the self-hosted backend.
...
Add the MCP to `opencode.json` (see `opencode.mcp.example.json`), then the agent calls it:

```
opencode run --pure --auto --model freetoken-local/deepseek-ai/DeepSeek-V4-Flash \
  "Use the web_search tool to find the latest vLLM release version and the source URL."
# -> v0.27.1 — https://github.com/vllm-project/vllm/releases   (self-hosted brain + Bedrock search)
```
