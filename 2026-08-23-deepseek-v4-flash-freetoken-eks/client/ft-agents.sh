#!/usr/bin/env bash
# agents.sh — one-shot launchers for the EKS-hosted agents (self-hosted DeepSeek-V4-Flash backend).
#
...
# (no upload API). `push` kubectl-cp's a local file into the pod (default agent: opencode) and, for
# video, ffmpeg-downsamples it first (the engine samples only a few frames, so shrinking client-side is
# faster). It prints the pod path; reference it in the TUI, e.g. opencode: `@/root/media/foo.png`.
...
#   ./agents.sh opencode -r <session-id>          # resume an opencode session
#   ./agents.sh opencode -p "summarize this repo"  # non-interactive prompt
...
# value comes from FT_* environment variables, and the context defaults to the current kubeconfig
...
KUBE_CONTEXT="${KUBE_CONTEXT:-${FT_KUBE_CONTEXT:-$(kubectl config current-context 2>/dev/null || true)}}"
NAMESPACE="${NAMESPACE:-${FT_NAMESPACE:-freetoken}}"
...
REGION="${REGION:-${FT_REGION:-$(printf '%s' "$_CTX_CLUSTER" | sed -n 's#^arn:aws:eks:\([^:]*\):.*#\1#p')}}"
REGION="${REGION:-${AWS_REGION:-${AWS_DEFAULT_REGION:-$(aws configure get region 2>/dev/null || echo ap-northeast-1)}}}"
: "${KUBE_CONTEXT:?no kube context available — set FT_KUBE_CONTEXT or select one with kubectl}"
...
cmd_setup() {
  CLUSTER_NAME="${CLUSTER_NAME:-${FT_CLUSTER_NAME:-}}"
  : "${CLUSTER_NAME:?setup needs a cluster name — set CLUSTER_NAME or FT_CLUSTER_NAME}"
  ...
  echo "[OK] setup done. Now:  ./agents.sh opencode | hermes | openclaw"
}
...
_bin_for() { case "$1" in opencode) echo opencode;; hermes) echo hermes;; esac; }
...
cmd_openclaw() {
  ...
  local url="http://127.0.0.1:${port}/#token=${OPENCLAW_TOKEN:-freetoken-demo-token}"
...
cmd_push() {
  local src="${1:?usage: push <local-file> [opencode|opencode|hermes]}"; local agent="${2:-opencode}"
...
Env: FT_KUBE_CONTEXT (default current), FT_NAMESPACE (default freetoken); region from the EKS ARN.
USAGE
}
