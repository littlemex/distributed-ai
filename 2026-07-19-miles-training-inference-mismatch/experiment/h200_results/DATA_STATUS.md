...
| `bf16_lr1e5` (batch `ctl_bf16lr5`) | 0.00063 (max) | **欠けていた対照実験**。bf16 KV + LR 1e-5 + no TIS。崩壊アームと LR を揃えて dtype だけを戻した |
...
## 実行ハードウェアの記録が不完全な点 (2026-08-04 に判明)

**この台帳は run ごとの実行ハードウェアを記録していない。** `verify_results.py` が読むのは
H200 クラスタ (distai-p5-ue2) の `/fsx` だけなので、ここに載る run は暗黙に H200 だと
読めてしまう。しかし実際には H100 (p5) で走った run も同じ `/fsx` に残っており、
少なくとも 1 件、一次記録どうしが食い違っている。

| 数値 | 記録 A | 記録 B | 判定 |
|---|---|---|---|
| baseline `mis_kl` 0.000632 | `P4_calibration.md`: **H100** 側の値 (H200 は 0.000627) | upstream の `docs/RESULTS.md` 冒頭: 「single **H200** x8 node」 | **未解決**。どちらかが誤記 |
| 3 seed collapse / TIS (`amplified_*`, `tis_*`) | upstream `docs/RESULTS.md`: 「30 steps each, on p5 (**H100** x8)」 | (この台帳には記載なし) | H100 と読むのが妥当 |
| 位置プロファイル、weight sync、対照実験 (`bf16_lr1e5`) | この台帳 (H200 クラスタの `/fsx` 由来) | — | H200 |

**運用への影響。** 崩壊アームの値 (H100) と対照実験 `bf16_lr1e5` の値 (H200) を並べた
比較は、厳密には dtype 以外にハードも変わっている。dtype の帰属という結論自体は
「崩壊するかしないか」という方向の差なので保たれるが、値の大小を比較する用途には使えない。
文書側でこの限定を書くこと。

**再発防止。** 今後 run を追加するときは、cell の env に実行クラスタ名を含めるか、
`exp_init` が記録する `environment.txt` (nvidia-smi の出力) を台帳に紐づける。
現状の `environment.txt` はバッチ単位でしか残っていないので、run 単位で辿れない。
...
## 欠けていた対照実験 (2026-08-04): LR ではなく dtype が崩壊を駆動している

崩壊アームは全て **KV dtype (bf16 → fp8_e5m2) と学習率 (1e-6 → 1e-5) を同時に**変えていた。
一方 bf16 の run は全て LR 1e-6 だったので、「LR 1e-5 だけで崩壊するのではないか」という
反論を潰す対照が 1 本も存在しなかった。登壇資料のレビューでこの穴が指摘され、実測した。

cell `bf16_lr1e5` (batch `ctl_bf16lr5`, job `raysubmit_CYSrPEXCzgTynap1`): bf16 KV、LR 1e-5、
no TIS、30 step、seed 1234。崩壊アーム `amplified_s1234` と**dtype 以外すべて同一**である。

step を揃えた比較:

| step | fp8_e5m2 mis_kl | fp8_e5m2 grad_norm | bf16 mis_kl | bf16 grad_norm |
|---|---|---|---|---|
...
| 29 (最終) | 0.808 | 0.0 (崩壊で advantage 全ゼロ) | 0.000558 | 0.116 |
| max | **12.59** | **57.5** | **0.00063** | **0.169** |

**結論: LR 1e-5 それ自体は崩壊を起こさない。** 同じ LR で dtype を bf16 に戻すと
`mis_kl` は約 0.0006 (良性水準) のまま動かず、`grad_norm` も 0.17 を超えない。
fp8 アームの max と比べると `mis_kl` で **約 2 万倍**、`grad_norm` で **約 340 倍**の差である。
したがって「fp8 で増幅された mismatch が崩壊を駆動する」という帰属は、
論文の Fig 13 に依存せず**自前の実測で支持できる**。

bf16 側は 30 step 完走し、reward は 0.562 で生存、`repetition_frac` も最大 0.031 で
生成は退化していない。**崩壊の兆候が 1 つも出ていない。**

verdict: `VERIFIED` (TensorBoard と driver が `mis_kl=0.0005578891` で一致)、
sanity screen も通過、`UNSCREENED` 項目なし。batch `ctl_bf16lr5`。

この対照が無かった間、記事とスライドは「dtype と LR を同時に変えた」ことを caveat として
明記してはいたが、交絡を実験で潰してはいなかった。潰した。
