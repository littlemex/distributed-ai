...
[EN]
This slide summarizes the three things to be aware of beyond the network when running GRPO at scale. The bottom bar is our actual measurement, from a miles run on 4B, H200 times two: one step is 119.2 seconds. Rollout generation dominates at 74.8 seconds, about 63 percent of the step. actor_train, the backward pass and optimizer step, is 16.4 seconds here at 4B — it grows with model size, roughly 38 seconds at 30B. The rest, about 27 seconds, is mostly the reference-model log-prob pass, about 22 seconds, plus sleep and wake-up overhead. And weight sync over the network is just 0.57 seconds, half a percent of the whole step. The network matters, but in this case it was not the dominant cost.
...
[JA]
このスライドはGRPOをスケールで動かすときにネットワーク以外で意識すべき3つのことをまとめています。下のバーは実際の今回の実測で、milesの4B、H200×2のrunです。1ステップが119.2秒です。ロールアウトの生成が74.8秒で支配的、ステップ全体の約63%です。actor_train、つまりbackwardとoptimizer更新は、ここでは4Bで16.4秒です。これはモデルサイズとともに伸び、30Bなら約38秒です。残りの約27秒は、大半が参照モデルのlog-prob計算で約22秒、それにsleepやwake-upのオーバーヘッドが乗ります。そしてネットワーク経由のweight syncはわずか0.57秒、ステップ全体の0.5%です。ネットワークは重要ですが、今回のケースでは支配的なコストではありませんでした。
...
## Slide 14 -- Our measurement: mismatch runs away without TIS, stays flat with it

[EN]
Here is what we actually measured, all 30 steps straight from TensorBoard. The red curve is TIS off: mis_kl starts at 0.032, is quiet through the first several steps, then climbs stage by stage -- past 0.4 around step 14, past 1 around step 17, and up to 31.8 by step 29. It runs away, roughly a thousand-fold. Notice the shape on this log axis: it is close to a straight line, which means exponential growth. That is the signature of a step-to-step positive feedback loop -- mismatch biases the gradient, the biased update drifts the policy, and that widens the mismatch on the next step. The blue curve is TIS on: it never leaves the floor, peaking around 0.15 and ending near 0.07, and the faint bands are the min-max over three seeds -- every collapse arm blows up, every TIS arm stays flat. Turning on the cap of 2.0 breaks that feedback loop.
One important clarification, because the title of the previous slide mentioned T squared. The paper's O(T squared) bound is about T as response length, within a single gradient step -- it bounds how much error one long sequence can carry. It does not predict how mis_kl grows across training steps. I did not measure per-step response length here, so I deliberately do not draw a T-squared curve on this step axis. And I only ran 30 steps, so "flat here" is not "flat forever."

[JA]
これが実際に測定したもので、全30ステップを TensorBoard からそのまま出しています。赤い曲線が TIS オフです。mis_kl は 0.032 から始まり、最初の数ステップは静かで、そこから段階的に登ります。ステップ14前後で 0.4 を超え、ステップ17前後で 1 を超え、ステップ29で 31.8、およそ千倍まで暴走します。この log 軸での形に注目してください。ほぼ直線、つまり指数的な増大です。これは step をまたいだ正のフィードバックループの兆候です。ミスマッチが勾配をバイアスし、バイアスした更新が方策をdrift させ、それが次のステップのミスマッチをさらに広げる、というループです。青い曲線が TIS オンです。30ステップを通じて床から離れず、最大でも 0.15 程度、最後は 0.07 付近で終わります。薄い帯は3シードの min-max で、崩壊アームは全部暴走し、TIS アームは全部平坦です。キャップ 2.0 を入れると、このフィードバックループが断ち切られます。
一つ大事な補足です。前のスライドのタイトルで T の2乗に触れたので明確にしておきます。論文の O(T の2乗) の上界は、T が応答長で、1回の勾配ステップの中の話です。1つの長い系列がどれだけ誤差を運びうるかの上界であって、mis_kl が学習ステップをまたいでどう増えるかは何も予言しません。このチャートはステップ軸なので、論文の T の2乗則ではなく、測定されたフィードバック発散として読んでください。そして30ステップしか回していないので、「ここで平坦」は「永遠に平坦」ではありません。
