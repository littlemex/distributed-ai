...
# ============================================================
# miles GRPO training entrypoint (runs on the Ray worker).
#
...
#   miles's per-model scripts (scripts/models/*.sh, inherited from slime) define
#   MODEL_ARGS as a bash ARRAY. `ray job submit` re-joins everything after `--`
#   with subprocess.list2cmdline and runs it through an outer `/bin/sh -c`
...
#   escaping bug impossible. This matches how the upstream launch scripts
#   (scripts/run-*.sh) expand ${MODEL_ARGS[@]} in the same shell that sourced it.
#
# Inputs:
#   $@                 : all train.py flags assembled by the recipe (scalar
#                        argv tokens, safely quoted by Ray).
#   env MODEL_SCRIPT   : the model script to source (e.g. qwen3-4B.sh), passed
#                        through the Ray runtime env by the recipe.
#   env SLIME_DIR      : framework install dir in the image (default /root/miles;
#                        variable name kept for parity with the upstream launcher).
# ============================================================

set -euo pipefail

# miles installs itself editable at /root/miles (the variable name is kept as
# SLIME_DIR for parity with the upstream launcher; only the default path moves).
