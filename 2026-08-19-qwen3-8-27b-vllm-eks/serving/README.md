...
| tool-calling / image / video | verified | not verified |
| Image | upstream `vllm/vllm-openai` | custom image required (see `sglang/image/`) |
| TPOT c=1 | ≈ 9.5 ms | ≈ 8.1 ms (raw bench, without the tool-calling path) |

TPOT is measured at `in512/out256`, `max-model-len 8192`, `--ignore-eos`; speculative-decoding speed
depends on the acceptance rate, so it varies with the prompt. At the full YaRN 1M context TPOT rises
because the 16 full-attention layers read a larger KV cache each token. The 1M setting is enabled and
a 375k-token needle is retrieved on the SGLang/DFLASH path; it is not RULER-evaluated.
...
| d | composed-config TPOT below vLLM | vs the vLLM baseline in `../experiments/mtp/results.md` | not met |
...
- `pool/nodepool-gpu-l40s.yaml` — the production on-demand pool (L40S x4). Experiment pools live under
  `../experiments/`.
- `models/qwen3.8-27b.yaml` — config.json-derived facts only; measured TPOT and its conditions live in
  `../experiments/`.
