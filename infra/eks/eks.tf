...
module "eks" {
  ...
  ################################################################################
  # NOTE: the Node Monitoring Agent is installed as a standalone aws_eks_addon
  # (observability.tf), NOT as an entry in this module `addons` map. Adding it
  # here re-evaluates the whole module dependency graph and forces the Karpenter
  # node IAM policy attachments to be replaced (policy ARNs go "known after
  # apply"), momentarily detaching live nodes' permissions. The standalone
  # resource (same pattern as the EFS/FSx CSI add-ons) avoids that. Its GPU-taint
  # toleration is passed via the add-on's configuration_values.
  addons = {
    vpc-cni = {
      before_compute = true
    }
