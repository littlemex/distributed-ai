...
# What "mcp exec" promises is narrow and easy to break: the command it wraps sees the servers, owns the
# exit code, and finds the tunnels closed again afterwards. The last part is the one a reader would
# never notice going wrong, because a leaked port-forward keeps working until the next reboot.
test_accelprof_client_mcp_exec_wraps_and_restores() {
  local client
  client="$(_ac_client)"
  [ -x "$client" ] || return 2
  local dir out fails=0
  dir="$(mktemp -d)"
  mkdir -p "$dir/bin"
  # A kubectl that lists the two Services and speaks port-forward's first line, which is where the real
  # local port comes from. It records its own pid so the test can check the process is gone afterwards.
  cat >"$dir/bin/kubectl" <<STUB
#!/usr/bin/env bash
args="\$*"
case "\$args" in
  *"get svc"*) printf 'analysis\nknowledge\n' ;;
  *port-forward*)
    for a in "\$@"; do case "\$a" in *:8080) spec="\$a" ;; esac; done
    port="\${spec%%:*}"
    [ -n "\${port}" ] || port=54321
    printf 'Forwarding from 127.0.0.1:%s -> 8080\n' "\${port}"
    printf '%s\n' "\$\$" >>"$dir/forward-pids"
    # Stands in for a tunnel that stays up until it is killed.
    sleep 60 &
    wait
    ;;
  *) ;;
...
STUB
  # A curl that answers the initialize probe, so that "the server answered" is exercised without a
  # server. Anything else it might be asked, it declines, which is what a wrong URL would look like.
  cat >"$dir/bin/curl" <<'STUB'
#!/usr/bin/env bash
case "$*" in
  *127.0.0.1:*/mcp*) exit 0 ;;
  *) exit 22 ;;
esac
STUB
  chmod +x "$dir/bin/kubectl" "$dir/bin/curl"
  : >"$dir/forward-pids"
  out="$(PATH="$dir/bin:$PATH" KUBE_CONTEXT=stub TMPDIR="$dir" "$client" mcp exec -- \
    sh -c 'printf "%s\n" "$ACCELPROF_MCP_CONFIG"; printf "analysis=%s\n" "$ACCELPROF_ANALYSIS_URL"; exit 7' 2>&1)"
  local rc=$?
  [ "$rc" = "7" ] || { printf 'FAIL the wrapped command owns the exit code, got %s\n' "$rc" >&2; fails=$((fails + 1)); }
  case "$out" in
    *'"accelprof-analysis"'*'"accelprof-knowledge"'*) ;;
    *) printf 'FAIL the config did not name both servers: %s\n' "$out" >&2; fails=$((fails + 1)) ;;
  esac
  # The URL has to carry the port kubectl reported, not the one that was asked for.
  case "$out" in
  ...
  done <"$dir/forward-pids"
  [ "$left" = "0" ] || { printf 'FAIL %s forward(s) were left running after exec returned\n' "$left" >&2; fails=$((fails + 1)); }
  [ -d "$dir/accelprof-mcp" ] && [ -n "$(ls -A "$dir/accelprof-mcp" 2>/dev/null)" ] &&
    { printf 'FAIL state was left behind in %s\n' "$dir/accelprof-mcp" >&2; fails=$((fails + 1)); }
  rm -rf "$dir"
  [ "$fails" -eq 0 ] || return 1
}
